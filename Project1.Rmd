...
my_table <- datatable(bicycle_df_trans,rownames = FALSE)
my_table
```
...
Now we will use contingency tables <https://en.wikipedia.org/wiki/Contingency_table> to look at the frequency distribution of several variables.  
...
ctab2 <- table(my_tibble$bike_race)
ctab2 <- kable_styling(kable(ctab2))
ctab2
#create 3rd contingency table
ctab3 <- table(my_tibble$crash_mont)
ctab3 <- kable_styling(kable(ctab3))
ctab3
```
...
ctab4 <- kable_styling(kable(ctab4))
ctab4
```
