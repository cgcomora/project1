...
g <- ggplot(data = my_tibble, aes(x = my_tibble$median_household_income,y = my_tibble$masters_degree_male))
g + geom_point() + geom_smooth(method = lm) +facet_wrap(~my_tibble$block_group) + labs(title = "Income and education by block group", y= "Males with Masters Degrees",x="Median Income")
```
...
```{r barplot, echo=TRUE, eval=TRUE, message=FALSE}
g <- ggplot(my_tibble,aes(x=class)) + geom_histogram(stat = "count", aes(fill = my_tibble$census_year))
g
```